    ...
    <div className="max-w-6xl mx-auto px-5 relative z-10">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 sm:gap-12 mb-16 sm:mb-20">
        <div className="md:col-span-2 text-center md:text-left">
          ...
          <p className="text-white/60 font-medium max-w-sm leading-relaxed mb-8 mx-auto md:mx-0 text-sm">
          ...
          <a href={`mailto:${companyInfo.supportEmail}`} className="inline-flex items-center justify-center md:justify-start gap-3 text-white/60 transition-[color] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] [@media(hover:hover)_and_(pointer:fine)]:hover:text-white">
            <Mail size={16} className="text-white/50" />
            <span className="font-bold text-xs">{companyInfo.supportEmail}</span>
          </a>
          {companyInfo.address && (
            <p className="mt-3 text-white/60 font-bold text-xs">{companyInfo.address}</p>
          )}
        </div>
        <div className="text-center md:text-left">
          <h4 className="text-[10px] font-black uppercase tracking-widest text-white/50 mb-6 sm:mb-8">Selskap</h4>
          <ul className="space-y-3 sm:space-y-4 text-white/60 font-bold text-sm">
      ...
      <div className="pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-white/50 text-center">
