...
/** Finner riktig Stripe-betalingslenke ut fra et plannavn ("BASIC", "Standard Pakke", …). */
export function getStripeLink(plan: string): string | undefined {
  const key = (plan || '').toUpperCase();
  if (key.includes('PREMIUM')) return LINKS.PREMIUM;
  if (key.includes('STANDARD')) return LINKS.STANDARD;
  if (key.includes('BASIC')) return LINKS.BASIC;
  return undefined;
}
