...
export const FaqItem: React.FC<{ q: string; a: React.ReactNode }> = ({ q, a }) => {
  return (
    <details className="group bg-white/80 backdrop-blur-sm border border-[#E9E4DA] rounded-2xl px-5 sm:px-6 ui-motion [@media(hover:hover)_and_(pointer:fine)]:hover:shadow-sm">
      <summary className="flex items-center justify-between gap-4 cursor-pointer list-none py-4 sm:py-5 text-[#1A1A1A] font-bold text-base sm:text-lg [&::-webkit-details-marker]:hidden">
        ...
        <ChevronDown
          size={20}
          className="text-[#5C574C] shrink-0 transition-transform duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] group-open:rotate-180"
        />
      </summary>
      <div className="pb-5 -mt-1 text-[#5C574C] font-medium leading-relaxed">{a}</div>
    </details>
  );
};
